...
This component receives input from the patient (via the Emotive device) and configuration commands from the control layer, and returns the current state the user is in (concentrating/not concentrating) and stores data during the training process.

\subsubsection{Component 1.1: Data Filter}
This component takes in the data from the Emotiv device and then performs some simple filtering on it. One of the most necessary filters for testing will be removing data if the accelerometer senses movement, since movement disrupts the signals. It will also provide simple filtering that will remove any data that has too much noise in it to be useful. This component then passes the data to the main EEG processor.
...
The UI Cycle Routine cycles through all of the available Interactive State Pages (2.3) one bby one, and receives input from the EEG processing to select one. 
