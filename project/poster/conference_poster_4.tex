...
can use to interact with their houses through the Insteon \cite{Insteon} Home Automation Systems (HAS) and an Emotiv\cite{Emotiv} EEG reader.
...
%-----------------------------------------------------------------
\headerbox{Machine Learning Model}{name=machinelearning,column=0,above=bottom}{

Sampled data from the Emotiv at 128 Hz over the course of 5 seconds with intervals of half a second to acquire the nuetral training data. To get the Accept Thought data, the same method was used after displaying a prompt to the user to ``think their Accept Thought''. Using these samples, A Fast Fourier Transformation (FFT) is applied to get 15 features from each sample. (Kyle Elaborate here). A Naiive Bayes Classifier is trained using the features extracted with the FFT. Once the classifier is trained, instances are tested using a rolling window of 128 Hz. When there are four accepted instances in a row, the current thought is considered to be the accept thought and the user selects.
...
At the application selection screen, the user will cycle through the options of the objects that they can interact with in the home automation system and can decide which object they want to interact with. When an object is chosen, the user will be directed to the interactive state screen for the selected objectwhich handles the changing of the state of the selected object. This could be some as simple as a state change from "on" to "off" or input that requires integer selection. From here, the user can either change the state or return to the previous screen. These three types of UI screens will make up the user interaction in the system. 
}
...
\headerbox{Conclusion}{name=conclusion,column=2,span=2,row=0,below=architecture}{
...
\item We showed that the Insteon home automation system can be used without movement from the user using a Machine Learning classifer on EEG data
\end{itemize}
}
