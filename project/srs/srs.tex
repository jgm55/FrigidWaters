...
commands to be sent to the home automation controller. The controller in turn
commands the home components, such as the television and lights.
...
are generated in response to promptings from the display. These impulses are
...
The system interfaces with both the Emotiv device and the Insteon PowerLinc
Modem via Universal Serial Bus (USB).
...
% XXX Don't know if I like "initial release." How else to say this?
...
% XXX We don't have a thermostat, so we shouldn't say we're going to control
% one. I guess we shouldn't reuse the number...?
...
% XXX I don't think these count...
%\textbf{SR505} We must comply to the input standards of the Emotiv device.
%
%\textbf{SR510} We must comply to the output standards of the 
...
% XXX 30 minutes (or any time duration) seems like a poor reference period.
% Unless this means "unintentionally activate"?
